...
- [decisions/history-storage.md](decisions/history-storage.md) — Why file/ZIP retained-history authority, disposable indexes named by schema (never migrated in place), retention and restore, and post-stop native capture; which parts native-only history superseded
...
- [decisions/legacy-native-import.md](decisions/legacy-native-import.md) — Why pre-key chats get one bounded exact-key import, its limits, and measured result; open decision on 0.6.7 Pi chats with no recorded ID; implementation details are in the architecture page
- [decisions/native-only-history.md](decisions/native-only-history.md) — Conversation history is the harness's native transcript: one read path, old runner history dropped (option C), rebuildable search and live run facts, combined-PR archive/browse/read fixes, one-time migration and late-binding repair policy, and prune rationale
...
- [concepts/model-resolution/agent-profiles.md](concepts/model-resolution/agent-profiles.md) — Profile loading from `.mars/agents/`, frontmatter fields, skill attachment, profile backup candidates
- [concepts/model-resolution/model-policies.md](concepts/model-resolution/model-policies.md) — `model-policies` typed selector rules, visibility flags, per-entry `no-fallback`, whole-profile backup scan
...
- [architecture/launch-system.md](architecture/launch-system.md) — `build_launch_context()` central seam; prepare/bind split (`prepare_launch_surface` + `bind_launch_context`), `PreparedLaunchSurface`, `CatalogSession`; three driving adapters, composition invariants; interactive starting-prompt delivery per harness
- [architecture/state-system/overview.md](architecture/state-system/overview.md) — Per-spawn state.json, context-resolved work items, atomic tmp+rename writes, `fcntl.flock` locking, reaper reconciliation
- [architecture/state-system/portable-history.md](architecture/state-system/portable-history.md) — Retained-history authority: file/ZIP retention, per-schema index files and mixed-version overlap, bounded index initialization, preview projection, native-only archive capture, reclaim, inert restore, retained-snapshot reads for restored and imported refs
...
- [architecture/legacy-native-import.md](architecture/legacy-native-import.md) — One-time exact import of legacy chat native keys: trigger, locking, recovery, candidate stores (incl. Pi interactive root), in-place OpenCode reads, deferral backoff, report mode, and cost
...
- [architecture/native-transcript-reads.md](architecture/native-transcript-reads.md) — Combined PR #534 implementation: one resolver and `NativeRead` reader, chat-named pN views, native-keyed FTS5 search projection (witness, `complete`, rebuild/status, measured cost)
- [architecture/attempt-facts-and-delivery.md](architecture/attempt-facts-and-delivery.md) — Emit path (hooks → fan-out → `note_event_delivered`), per-harness attempt folds and report precedence, Pi lifecycle sidecar, the retired runner stream, history-blind read trap
...
- [lessons/native-session-identity.md](lessons/native-session-identity.md) — Native-session work lessons: seam defects green suites missed, terminal status versus process exit, removing carriers, once-only markers, restructure before stacking, projections that re-derive the authority's rule, `pytest -x` in slice gates, argv-gated `sitecustomize` missing `python -m` children, a diagnostic flag discarding authoritative cost, and correcting live-probe briefs and input protocol; argv reorder let a variadic flag swallow the prompt; in-place projection upgrade broke the running old build
- [lessons/spawn-lane-operations.md](lessons/spawn-lane-operations.md) — Spawn lanes that fail invisibly: declared backups do not cover a mid-run auth failure (pin `-m` to a declared backup), a Claude-routed lane can end its turn before a background gate finishes
- [lessons/dogfooding-pr-builds.md](lessons/dogfooding-pr-builds.md) — Running unreleased builds safely: never on a real runtime root, schema bumps and rollback, nested-env pinning and `-C`, `uv tool install` relinking, scratch outside synced repos, masked push exit codes and SSH keepalive; probing the upgrade path with a genuinely old build (`uvx --isolated`) and mixed-version overlap
