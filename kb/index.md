...
- [architecture/state-system/portable-history.md](architecture/state-system/portable-history.md) — Retained-history authority: file/ZIP retention, bounded index initialization, preview projection, native-only archive capture, reclaim, inert restore
...
- [lessons/native-session-identity.md](lessons/native-session-identity.md) — Native-session work lessons: seam defects green suites missed, terminal status versus process exit, removing carriers, once-only markers, restructure before stacking, projections that re-derive the authority's rule, `pytest -x` in slice gates, argv-gated `sitecustomize` missing `python -m` children, a diagnostic flag discarding authoritative cost, and correcting live-probe briefs and input protocol
...
- [lessons/dogfooding-pr-builds.md](lessons/dogfooding-pr-builds.md) — Running unreleased builds safely: never on a real runtime root, schema bumps and rollback, nested-env pinning and `-C`, `uv tool install` relinking, scratch outside synced repos, masked push exit codes and SSH keepalive
