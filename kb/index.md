...
- [decisions/native-only-history.md](decisions/native-only-history.md) — Conversation history is the harness's native transcript: one read path, old runner history dropped (option C), native-keyed FTS5 trigram search with exact verification, run facts from the live fold, the metadata index calling the shared fold, retention decisions
...
- [architecture/attempt-facts-and-delivery.md](architecture/attempt-facts-and-delivery.md) — `EmitOutcome` emit path, per-harness attempt folds and report precedence, Pi lifecycle sidecar, what writers PR 3 still has to delete, history-blind test mode
