...
`session archive --prune-runner-history` is an explicit maintenance command, not a
transcript read path. Its operator contract and implementation safeguards are in
[session archive pruning](../operations/session-archive-pruning.md); the rationale
and measurements are in the [native-only history decision](../decisions/native-only-history.md#the-prune-rule).
