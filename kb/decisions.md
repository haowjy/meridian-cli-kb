...
|---|---|---|---|
| 2026-09-24, D-native-source-argument-admission | R1 fresh owner accepted on unmerged branch; R2/G/B3c open | Keep ordinary Mars at its existing late point; admit fresh/`--from` raw with the selected adapter and retained controls before preparation. Typed saved raw needs before-history checks; final emission needs independent validation. | [Native source argument admission](decisions/native-source-argument-admission.md) |
| 2026-09-23, D-native-session-identity | Settled intent; unmerged enabling slices, not shipped | Pin each cN to one exact native key; record actual run entry/exit; read native history and stop new runner-stream writes without changing initiation modes. R1 fresh admission is bounded fake-only evidence. | [Native session identity](decisions/native-session-identity.md) |
...
| 2026-09-14, D-native-transcript-snapshot | Superseded as future write-path policy; earlier feature-branch work unreleased | Earlier stream-plus-snapshot choice; the approved [native-session-identity design](decisions/native-session-identity.md) removes new runner-stream writes while retaining qualified snapshots and inert legacy evidence. | [History storage](decisions/history-storage.md#d-native-transcript-snapshot) |
...
| 2026-09-11, D-history-file-authority | Intended; retention and preview approved on feature branch, not released | Retained transcripts and ZIPs are independently readable file authority; SQLite metadata and previews are disposable one-way projections. | [History storage](decisions/history-storage.md#d-history-file-authority) |
...
| Native session identity | Settled cross-harness binding and native-history contract; partial unmerged implementation | [decisions/native-session-identity.md](decisions/native-session-identity.md) |
| Native source argument admission | Late fresh-route C and bounded R1 adapter admission; typed/public/final consumer and native qualification remain open | [decisions/native-source-argument-admission.md](decisions/native-source-argument-admission.md) |
