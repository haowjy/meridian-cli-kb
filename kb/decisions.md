...
|---|---|---|---|
| 2026-09-25, D-native-only-history | Settled; PR 2 landed in draft PR #526 (stacked on #520); PR 3 landed on `feat/stop-runner-history` (stacked on #526); neither on `main` | Transcript reads, search and run facts use only the chat's native transcript; old runner history is dropped, not decoded (user: option C). Search is a disposable native-keyed FTS5 trigram projection with exact re-verification; `complete` means every in-scope source searched and no cap truncation. The metadata index calls the shared fold step. Run facts come from live per-attempt folds; incomplete facts drop only generic usage. The < 1 s warm-search target is unmet (CLI import cost, #527). PR 3 stopped writing runner history, migrates dogfood rows once through `meridian doctor`, and adds an explicit, dry-run-first `session archive --prune-runner-history`. | [Native-only history](decisions/native-only-history.md) |
| 2026-09-24 (extended 2026-09-25), D-native-session-identity | Settled; implemented on unmerged draft PR #520 (head `2eddcd68`, installed by the user for dogfooding), including the legacy import and the P0–P5 foundation restructure | Each cN binds one immutable `(harness, store, id)`; `--continue cN` resumes exactly it or fails with a typed reason. Entry is a harness-guaranteed exact target bound before exec. One binding rule: `Bound`/`Same`/`Conflict`, conflicts never applied. One drift rule: only an attempt's first owned signal can contradict a pre-exec fact. One runner pipeline concludes after teardown and writes one `run_boundary` (`verified`/`unresolved`/`mismatch`). `continue_chat_id` is the post-run continue rule. Legacy chats were imported once. | [Native session identity](decisions/native-session-identity.md) |
