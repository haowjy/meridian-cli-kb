...
|---|---|---|---|
| 2026-09-24, D-native-source-argument-admission | Fresh-route C chosen; effect gate blocks integration | Admit typed source before source-dependent replay; retain one ordinary policy for fresh/`--from` inferred-harness benign raw, then parse the actual adapter. R0 is behavior-preserving only. | [Native source argument admission](decisions/native-source-argument-admission.md) |
| 2026-09-23, D-native-session-identity | Settled intent; unmerged enabling slices, not shipped | Pin each cN to one exact native key; record actual run entry/exit; read native history and stop new runner-stream writes without changing initiation modes. | [Native session identity](decisions/native-session-identity.md) |
...
| Native source argument admission | Bounded adapter grammar and chosen fresh-route C; installed-resolver effect gate, no qualified transport | [decisions/native-source-argument-admission.md](decisions/native-source-argument-admission.md) |
