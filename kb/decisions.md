...
**Native hook fragments replace command synthesis (2026-07)**
Supersedes D90's schema while keeping its principles. Authors write per-target native fragment files (the harness's own JSON hook shape verbatim, or TypeScript for file-mode targets); mars stops owning any hook entry schema. Two modes: MergeJson (Claude, Codex, Cursor) merges entries into config files; File (OpenCode, Pi) places substituted `.ts` plugins. Lock records exact emitted JSON for structural removal, ending the residue-cleanup class. Two-surface ownership model: path ownership via `OutputRecord` keyed on `(target_root, dest_path)` for files; config-entry ownership via `ConfigEntryRecord` with recorded `emitted_json` for merge entries. Retention seam (`surface_ownership/retention.rs`) enforces uniform failure handling per `(target_root, surface)` pair, making wrong states unrepresentable rather than checked. See [package-management decisions](decisions/package-management.md#d91-native-hook-fragments-replace-command-synthesis-2026-07).
