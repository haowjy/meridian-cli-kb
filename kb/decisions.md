...
| **Model resolution** | Alias resolution, routing context, profile schema, resolve-once, `MERIDIAN_HARNESS` spawn-local semantics (D52–D57); agent overlay config, canonical compiler, Meridian-vs-Mars ownership, candidate-chain semantics (D72–D75); model prompting guidance with agent-first resolution, canonical `models prompting` command name (D79) | [decisions/model-resolution.md](decisions/model-resolution.md) |
