...
|---|---|---|---|
| 2026-09-25, D-native-only-history | Settled; PR 2 (`feat/native-reads`) in progress, PR 3 not started | Transcript reads, search and run facts use only the chat's native transcript; old runner history is dropped, not decoded (user: option C). Search is a disposable native-keyed FTS5 trigram projection with exact re-verification. The metadata index calls the shared fold step. PR 3 stops writing runner history. | [Native-only history](decisions/native-only-history.md) |
