...
**model-invocable agent visibility filter at inventory prompt boundary (D-model-invocable, 2026-05 PR #208)**
`model-invocable: false` in agent profile frontmatter is enforced by `build_agent_inventory_prompt()`, not by the catalog scanner. The scanner remains policy-free; the inventory prompt is the model-facing boundary. Explicit CLI invocation ignores this field. See [launch decisions](decisions/launch.md#d-model-invocable-filter-at-inventory-prompt-boundary-not-at-catalog-scan).
...
| **Model resolution** | Alias resolution, routing context, profile schema, resolve-once, `MERIDIAN_HARNESS` spawn-local semantics (D52–D57); agent overlay config, canonical compiler, Meridian-vs-Mars ownership, candidate-chain semantics (D72–D75) | [decisions/model-resolution.md](decisions/model-resolution.md) |
