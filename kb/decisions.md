...
|---|---|---|---|
| 2026-09-24 (extended 2026-09-25), D-native-session-identity | Settled; exact entry for Pi/Claude/Codex/OpenCode and Pi/Claude exit mapping implemented on unmerged PR #520 branches; native readers and runner-history removal not started | Each cN binds one immutable `(harness, store, id)`; `--continue cN` resumes exactly it or fails with a typed reason. Entry evidence is a harness-guaranteed exact target bound before exec; one recorded source key; identity only from qualified events; exit observed (Pi quit, Claude successor) or unresolved, never a rebind. Replaces the observed-entry-only rule and the Pi RPC-primary plan. | [Native session identity](decisions/native-session-identity.md) |
