...
|---|---|---|---|
| 2026-09-23, D-native-session-identity | Settled intent; enabling slices reviewed on unmerged branch, not shipped | Pin each cN to one exact native key; record actual run entry/exit; read native history and stop new runner-stream writes without changing initiation modes. | [Native session identity](decisions/native-session-identity.md) |
