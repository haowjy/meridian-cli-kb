...
|---|---|---|---|
| 2026-09-23, D-native-session-identity | Approved technical design; unimplemented, source work not authorized | Pin each cN to one exact native key; record actual run entry/exit; read native history and stop new runner-stream writes without changing initiation modes. | [Native session identity](decisions/native-session-identity.md) |
...
| Native session identity | Approved, unimplemented cross-harness binding and native-history contract | [decisions/native-session-identity.md](decisions/native-session-identity.md) |
