...
Foreground `spawn` and single-spawn `spawn wait` now default to compact report-first text: one-line status, blank line, report body (or `(no report)`), then the transcript inspection command. The transcript pointer (`meridian session log <id>`) is primary output, not metadata — it is the escape hatch when the report is incomplete. `--metadata` adds inline accounting detail (model, cost, tokens, duration, report path); `--verbose` remains debug verbosity. Both are separate concerns. Agent mode matches human mode by default (text, not JSON); explicit `--format json` includes the same primary content structurally. See [concepts/spawn-output-contract.md](concepts/spawn-output-contract.md).

**Curated per-command JSON projections over --fields/jq (triage-designs-impl #113, PR pending, 2026-07)**
CLI JSON output for `spawn show`, `spawn wait`, `work show`, and `hooks run` uses curated per-command default projections via `CLIOutputProtocol` / `to_cli_wire()`. Heavy fields (`report_body`, raw `stdout`/`stderr`) are opt-in via explicit flags (`--full`, `--verbose`). `harness_session_id` dropped from `spawn show` JSON (human-approved divergence from issue #113). `--fields` and jq-like query languages rejected: composable later on top of projections if needed, but the noisy default was the actual bug. See [concepts/spawn-output-contract.md](concepts/spawn-output-contract.md).

**Single idempotent terminal owner for connection close events (triage-designs-impl #449, PR #467, 2026-07)**
Codex backend-exit watcher and WS reader race on death detection. `_emit_connection_closed_once()` guards emission with a lock: at most one close event, sole sentinel ownership, first enriched payload wins. Expected shutdown cancels the watcher before terminating the backend. Post-connect exit-code/stderr enrichment achieves parity with Claude/OpenCode. PGID reachability probe and shared `scope_liveness()` projection replace flat root-PID liveness in all three reaper consumers. See [codebase/harness-adapters.md](codebase/harness-adapters.md) and [architecture/process-scope.md](architecture/process-scope.md).

**Trust-ordered transcript resolution chain for Claude sessions (triage-designs-impl #171, PR pending, 2026-07)**
`resolve_session_file()` seam widened with `config_root_hint` parameter. Claude adapter searches a deduped trust-ordered root chain: recorded `claude_config_dir` → canonical `~/.claude` → ambient `CLAUDE_CONFIG_DIR`. Threaded through all tracked ref forms (bare session id, chat id, spawn id, corpus search, session repair). Non-Claude harnesses accept and ignore the hint. Persisting a transcript path rejected (staleness when re-materialized). Canonicalizing config dir at creation rejected (breaks overlay isolation). Required hint params force callers to choose rather than masking missed callers with `= None` defaults. See [codebase/session-operations.md](codebase/session-operations.md).

**Native hook passthrough replaces universal event vocabulary (mars-agents PR #133, 2026-07)**
Mars hook compilation switches from a universal event vocabulary (author writes `session.end`, mars translates per harness) to native passthrough: `hook.toml` declares harness-native event names in per-target tables; mars validates against per-target allowlists and passes through verbatim. Motivation: audit found 6 of 12 emitted mappings wrong (dead Claude `SessionStop`, per-turn Codex `Stop`, 4 fabricated OpenCode mappings). Cross-harness hook portability retired as a deliberate non-goal: zero demand, and hooks were never actually portable (payloads, matchers, and output protocols stayed harness-native). Codex `SessionEnd` excluded from allowlist after runtime probe proved it non-functional in 0.144.4. See [package-management decisions](decisions/package-management.md#d90-native-hook-passthrough-replaces-universal-event-vocabulary-mars-agents-pr-133-2026-07).

**Native hook fragments replace command synthesis (2026-07, PR #147)**
Supersedes D90's schema while keeping its principles. Authors write per-target native fragment files (the harness's own JSON hook shape verbatim, or TypeScript for file-mode targets); mars stops owning any hook entry schema. Two modes: MergeJson (Claude, Codex, Cursor) merges entries into config files; File (OpenCode, Pi) places substituted `.ts` plugins. Lock v3 records per-output lifecycle state (`Installed` with checksum, `PendingDeletion` without) scoped by `(target_root, dest_path)`, with exact emitted JSON for structural removal. Deletion authority and replacement authority are distinct: either state authorizes deletion, but only `Installed` authorizes overwriting an existing path; `PendingDeletion` at an existing path is an unmanaged collision, adoptable with `--force`. Retention seam (`surface_ownership/retention.rs`) enforces uniform failure handling per `(target_root, surface)` pair, making wrong states unrepresentable rather than checked. v2 locks are promoted by consulting disk state (the only point where legacy inference is correct). See [package-management decisions](decisions/package-management.md#d91-native-hook-fragments-replace-command-synthesis-2026-07).

**Shape-A recovery seam: halt before compilation when hook surfaces are unreadable (2026-07, PR #147)**
Recovery commands persist intent mutations and halt with exit 2 before the compiler when any resolved source has an unreadable hook surface. Strict sync remains the sole materializer. Two prior attempts to thread a "frozen" exception through the pipeline failed with rising severity: omission-as-absence (round 4: destroyed unrelated deps' installed state) and freeze-as-carried-state (round 5: leaked at 3 of N persistence sites). Shape A deletes the exception category entirely -- no diff/plan/target/lock exception paths at all. The gate is a single consumer of `unreadable_hook_surfaces` at the reader/compiler boundary in `sync::execute`. Corrupt locks are rebuilt in memory under the sync flock; on-disk bytes preserved until finalization. See [package-management decisions](decisions/package-management.md#d92-shape-a-recovery-seam----halt-before-compilation-when-hook-surfaces-are-unreadable-2026-07).

**Two-phase params.json living document (triage-designs-impl #334, PR #466, 2026-07)**
`params.json` becomes a two-phase living document: early `phase:"request"` write at prepare time (unchanged position, preserves pre-bind failure-window coverage), post-bind atomic refresh with `phase:"bound"` + `bound_work_dir`/`bound_task_dir` sourced from bound `run_params`. Request-metadata snapshot threaded through both launch paths. Rejected alternatives: post-bind-only write (loses failure-window coverage), second file `params.bound.json` (doubles artifact surface). `phase` describes what the file contains, not a lifecycle guarantee. `state.json` remains the lifecycle authority. See [architecture/state-system.md](architecture/state-system.md).
...
| **Package management** | Mars compiler, skill schema, `.agents/` elimination, targeting, sync, bootstrap docs, collision resolution, convention-based source discovery, upgrades-available direct-deps fix, init command contract, content scan pattern, honest field naming (D35–D40, D50–D51, D58–D63, D71, D77–D79, D87); launch-bundle system: Mars/Meridian ownership, native-config passthrough, tool-policy preservation, Cursor experimental, Pi future, Gemini out-of-scope (D80–D85); native hook passthrough, fragment authoring, and recovery seam (D90–D92) | [decisions/package-management.md](decisions/package-management.md) |
