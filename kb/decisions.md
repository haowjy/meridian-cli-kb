...
|---|---|---|---|
| 2026-09-24, D-native-session-identity | Settled; phase 1 (binding + Pi exact identity) implemented on unmerged branch; Claude/Codex/OpenCode lane in progress | Each cN binds one immutable `(harness, store, id)`; `--continue cN` resumes exactly it or fails. Entry evidence is a harness-guaranteed exact target bound before exec; discovery is never evidence. Replaces the observed-entry-only rule and the Pi RPC-primary plan. | [Native session identity](decisions/native-session-identity.md) |
