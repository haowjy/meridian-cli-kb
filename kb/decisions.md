...
|---|---|---|---|
| 2026-09-24 (extended 2026-09-25), D-native-session-identity | Settled; exact entry for Pi/Claude/Codex/OpenCode and Pi exit mapping implemented on unmerged draft PR #520 (whole-change review passed); native readers and runner-history removal not started | Each cN binds one immutable `(harness, store, id)`; `--continue cN` resumes exactly it or fails with a typed reason. Entry evidence is a harness-guaranteed exact target bound before exec; one recorded source key; identity only from qualified events; header-validated locators; typed mismatch/unavailable refusals; exit observed (Pi quit) or unresolved (Claude), never a rebind. Pre-existing chats get an exact, header-validated key once through a one-time legacy import (user decision 2026-09-25; slice branch merging into #520). Replaces the observed-entry-only rule and the Pi RPC-primary plan. | [Native session identity](decisions/native-session-identity.md) |
...
| Native session identity | Immutable chat→native key, exact-target entry evidence, no discovery; native transcripts are content authority | [decisions/native-session-identity.md](decisions/native-session-identity.md) |
