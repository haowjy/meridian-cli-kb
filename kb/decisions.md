...
|---|---|---|---|
| 2026-09-24, D-native-source-argument-admission | R1 fresh owner accepted; R2a metadata built; R2b/R2c/G/B3c open | Keep ordinary Mars at its existing late point; admit fresh/`--from` raw with the selected adapter. Typed raw must be checked before history, and final emission independently validated. | [Native source argument admission](decisions/native-source-argument-admission.md) |
| 2026-09-23, D-native-session-identity | Settled intent; unmerged enabling slices, not shipped | Pin each cN to one exact native key; record actual run entry/exit; read native history and stop new runner-stream writes. R2a joins admitted metadata exactly; no tracked transport qualified. | [Native session identity](decisions/native-session-identity.md) |
...
| Native session identity | Settled cross-harness binding and native-history contract; R2a metadata join built, tracked transport still refused | [decisions/native-session-identity.md](decisions/native-session-identity.md) |
| Native source argument admission | Late fresh-route C and bounded R1 adapter admission; R2a metadata does not close typed/public/final consumer gates | [decisions/native-source-argument-admission.md](decisions/native-source-argument-admission.md) |
