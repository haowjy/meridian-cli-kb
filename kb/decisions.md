...
|---|---|---|---|
| 2026-09-24, D-native-source-argument-admission | Bounded grammar policy settled on unmerged branch; integration stopped | Normalize only exact adapter-local native selectors, preserve validated non-session overrides, and stop at the unresolved actual-route/source-admission cycle rather than silently reduce typed continuation. | [Native source argument admission](decisions/native-source-argument-admission.md) |
| 2026-09-23, D-native-session-identity | Settled intent; unmerged enabling slices and B3b design STOP, not shipped | Pin each cN to one exact native key; record actual run entry/exit; read native history and stop new runner-stream writes without changing initiation modes. | [Native session identity](decisions/native-session-identity.md) |
...
| Native source argument admission | Bounded adapter grammar and route/admission STOP; no qualified transport | [decisions/native-source-argument-admission.md](decisions/native-source-argument-admission.md) |
