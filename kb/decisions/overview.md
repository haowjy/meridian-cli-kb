...
| [native-session-identity.md](native-session-identity.md) | Settled native-key/history contract; unmerged enabling slices and reviewed behavior-preserving R0, with chosen fresh-route C still blocked on resolver effects and no qualified tracked transport or shipped cutover |
| [native-source-argument-admission.md](native-source-argument-admission.md) | Typed-source ordering, bounded raw grammar, chosen fresh-route C, and its installed-resolver effect gate |
...
| [tui-framework.md](tui-framework.md) | Session-browse framework choice: prompt_toolkit full-screen Application (D-tui-prompt-toolkit) |
