...
| [native-session-identity.md](native-session-identity.md) | Settled native-key/history contract and source-selection policy; unmerged branch has enabling slices and a B3b design STOP, with no qualified tracked transport or shipped cutover |
| [native-source-argument-admission.md](native-source-argument-admission.md) | Bounded raw-selector policy, adapter-local grammar, and unresolved route/admission ordering |
