...
| **Extension** | A named, surface-aware command unit declared as an `ExtensionCommandSpec`. Extensions are the single source of truth for all user-facing operations. The CLI and MCP server consume the same extension registry; the HTTP surface is archived. | [concepts/extension-system.md](concepts/extension-system.md) |
...
| **History ID** | A relocation-stable UUID identifying one portable transcript. Unlike runtime-local `pN` and `cN` aliases, it does not change across archive, import, restore, or local alias remapping. | [architecture/state-system/portable-history.md](architecture/state-system/portable-history.md) |
...
| **Portable history record** | One readable JSONL transcript plus the file-backed identity, relationship, and recovery facts needed to transfer or reconstruct it. Loose files and verified immutable ZIPs are authority; SQLite is only a disposable projection. | [architecture/state-system/portable-history.md](architecture/state-system/portable-history.md) |
