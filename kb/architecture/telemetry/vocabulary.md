...
|---|---|---|
| **Debug tracer** | A per-spawn structured JSONL logger (`DebugTracer`) that writes harness wire events to a per-spawn debug file. Best-effort; self-disables on first failure. Activated by `MERIDIAN_DEBUG=1` at the lifecycle observer level, not through this class. | [overview.md](overview.md) |
