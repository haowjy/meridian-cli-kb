...
|---|---|---|
| **`/ps`** | `managed-bash` | List bash records (mechanism view). Shows only `b-*` records for the current session. |
| **`/ps:b <id>`** | `managed-bash` | Background a running foreground bash mid-flight. |
| **`/ps:kill <id>`** | `managed-bash` | Terminate a tracked bash. |
| **`/ps:logs <id>`** | `managed-bash` | Show log tail for a bash record. |
| **`/mspawn`** | `meridian-spawn-watch` | List spawn records (policy view). Shows only `p-*` records correlated to this session via `originating_bash_id`. |
| **`/mspawn:wait <id>`** | `meridian-spawn-watch` | Block on a spawn completion. |
| **`/mspawn:cancel <id>`** | `meridian-spawn-watch` | Cancel a spawn (proxies `meridian spawn cancel`). |
| **`/mspawn:show <id>`** | `meridian-spawn-watch` | Detail view with lifecycle + report + log tail (`meridian session log <id> -n 20` default). |
| **`/mspawn:log <id>`** | `meridian-spawn-watch` | Log dock overlay for just the session log. |
...
|---|---|---|
| **`originating_bash_id?: string`** | Field on spawn records. Populated at creation time from `MERIDIAN_PI_BASH_ID` if set in the environment. Used by `meridian-spawn-watch` to filter `/mspawn` rows to spawns originating from the current session's bash invocations. The detection signal is disk state + env, never argv parsing. | [../pi-lifecycle.md](../pi-lifecycle.md) |
...
| **`killed`** | Terminated externally via `bash_manage kill`, `/ps:kill`, `meridian spawn cancel`, or `/mspawn:cancel`. |
...
|---|---|---|
| **`pi-bash/<spawn-id>/bash-records.json`** | Per-spawn aggregate file for bash records. Written by `managed-bash`, watched by Python quiescence checker to evaluate condition (2) of the revised quiescence rule. | [../pi-lifecycle.md](../pi-lifecycle.md) |
